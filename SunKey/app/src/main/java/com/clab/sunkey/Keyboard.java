...
final class Keyboard {
    ...
    private final SunKeyService mSunKeyService;
    ...
    private Keyboard(SunKeyService sunKeyService, int viewResId,
                     SparseArray<String> keyMapping) {
        this.mSunKeyService = sunKeyService;
...
//        createKeyIdArray();
    ...
    static Keyboard sunkey(SunKeyService sunKeyService) {
        ...
        return new Keyboard(sunKeyService, R.layout.keyboard_sunkey, keyMapping);
    }

    static Keyboard sunkey_num(SunKeyService sunKeyService) {
        SparseArray<String> keyMapping = new SparseArray<>();
        keyMapping.put(R.id.key_pos_wave_mark, "~");
        keyMapping.put(R.id.key_pos_0_1, "wW+`");
        keyMapping.put(R.id.key_pos_0_2, "rR×\\");
        ...
        return new Keyboard(sunKeyService, R.layout.keyboard_sunkey_num, keyMapping);
    }
    ...
    private void handleTouchDown(String data, int index) {
        if (mDataBaseHelper.getSettingValue(mCustomVariables.SETTINGS_USE_VIBRATION_FEEDBACK)) {
            mSunKeyService.getVibratorService().vibrate(
        ...
        mSunKeyService.handleTouchDown(getTextFromFuncKey(data));
    }
    ...
    private void handleGestureInput(String data) {
        ...
        mSunKeyService.handleTouchDown(
    ...
    void initializeDataBaseHelper() {
        mDataBaseHelper = mSunKeyService.getDataBaseHelper();
    }
    ...
    private int dpToPx(float dp) {
        float density = mSunKeyService.getResources().getDisplayMetrics().density;
    ...
    private void continueInputHandler() {
        ...
        mSunKeyService.handleTouchDown(getTextFromFuncKey(mCurrentClickedKey));
    }
